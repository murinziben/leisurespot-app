/**
 * Lucide inline SVG helper for JS-generated HTML.
 * Returns a complete <svg> string for use inside innerHTML assignments.
 * For static HTML templates, prefer <i data-lucide="name"> + lucide.createIcons().
 */
...
const _ICON_PATHS = {
  ...
  'frown':            '<circle cx="12" cy="12" r="10"/><path d="M16.5 16s-1.5-2-4.5-2-4.5 2-4.5 2"/><line x1="9" x2="9.01" y1="9" y2="9"/><line x1="15" x2="15.01" y1="9" y2="9"/>',
};

function icon(name, size = 16, extraStyle = '') {
  const paths = _ICON_PATHS[name] || _ICON_PATHS['info'];
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:middle;flex-shrink:0${extraStyle ? ';' + extraStyle : ''}">${paths}</svg>`;
}

function starSvg(filled, size = 13) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="${filled ? '#E8711A' : 'none'}" stroke="${filled ? '#E8711A' : '#d1d5db'}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:middle"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>`;
}

function renderStarRow(rating, size = 13) {
  return Array.from({ length: 5 }, (_, i) => starSvg(i < Math.floor(rating), size)).join('');
}
